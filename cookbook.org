...
drop the time dependence and refer to the number of accounts with \(N\).

At a given time \(t\), a transaction \(j\) is defined as \(\Delta_{j,t} =
(\Delta^{i}_{j,t})_{1 \leq i \leq N}\) where \(\Delta^{i}_{j,t}\) is the
transaction's posting against account \(i\) at time \(t\).  The fundamental
accounting equation requires that the transaction is balanced i.e.: for a
transaction \(j\) occurring at time \(t\) we have \[\sum_{i=1}^N\Delta^{i}_{j,t} =
0\]
...
- I buy a coffee for 5 EUR with my credit card. My assets do not change.
  ...
  net worth, i.e. a decrease in equity in absolute terms. In practice, the
  coffee expense will be booked against an equity retained earnings account as a
  positive posting, i.e. a reduction in equity in absolute terms.
...
This is done by creating a transaction that will move the balance held in income
and expenses accounts into an equity account (traditionally called retained
earnings).
...
Suppose we use the below representation to record the purchase of 1 LQQ ETF with
euros.
...
zero. If you buy 1 LQQ with X EUR you are effectively adding 1 LQQ and removing
X EUR. The sum of the postings above is 1 LQQ + (-X EUR) which technically does
not add up to 0. *The books do not balance!* It looks like 1 LQQ was created out
of thin air while X EUR were destroyed.
...
2024-10-02 * Buy LQQ ETF (Amundi Nasdaq-100 Daily (2x))
    ...
    Equity:Conversion:BourseDirect:LQQ            -1 LQQ
...
account balance (converted at cost to the base currency) must be zero. This
allows for strict checks.
...
only in value but in every specific com modity involved in a transaction.

** Recording a purchase/sale with cost
While the use of conversion accounts guarantees the strict verification of the
balancing equation, it is cumbersome to maintain. The postings to conversion
accounts can always be generated with ~--infer-equity~. The following representations
...
It is preferable to use the =@ UNITCOST= notation rather than =@@ TOTALCOST=.
...
as follows:
- use the original cost price.
- manually calculate the capital gains recorded as a revenue.

#+begin_src ledger
2025-02-12 * Sell LQQ ETF
    Assets:FR:BourseDirect:PEA:ETF:LQQ            -1 LQQ @ 1037.90 EUR  ; the original cost
    Assets:FR:BourseDirect:Cash              1197.50 EUR
    Expenses:Finance:Fees                       2.50 EUR
    Income:CapitalGains                      -162.10 EUR  ; the capital gain from the sale
#+end_src
...
* Closing yearly journals
Check first that the sum of all balances is equal to zero.
#+begin_src
hledger -f 2024.journal bal -V
#+end_src

If not, most likely it is due to missing equity postings.
To identify missing equity postings, use:
#+begin_src sh
hledger -f 2024.journal bal --gain
#+end_src

To ensure the accounting equation is verified, income and expenses must be
consolidated into equity (retaining earnings).
#+begin_src sh
hledger -f 2024.journal close --retain --close-acct=Equity:Retained-Earnings -e 2025 -x
#+end_src

Check the net is zero
#+begin_src sh
hledger -f 2024.journal bse --layout=tall
#+end_src

Close the year
#+begin_src
hledger -f 2025-closed.journal close --close type:ALEV not:acct:Equity:Conversion:EUR-GBP --close-acct=Equity:Opening-Balances -e 2026 -x --show-costs
#+end_src

Open the next year
#+begin_src sh
hledger -f 2025-closed.journal close --open type:ALEV not:acct:Equity:Conversion:EUR-GBP --close-acct=Equity:Opening-Balances -e 2026 -x --show-costs
#+end_src

* Currency exchange
Suppose I buy EURs with GBPs. This can be represented as a trade as discussed above.
#+begin_src ledger
...
However, this will attach a cost to the account and as a result a potential gain
when using bal --gain.

This is where it may make sense to use equity postings to represent a simple
physical swap of cash.
...
* Receivables
** Loans
Suppose I lend money to someone and expect a refund later. The amount lent
should be held in an receivables account.

#+begin_src ledger
2026-02-08 * Loan to Florent
    Assets:Receivables:Florent                   100 EUR
    Assets:FR:BforBank:Compte-Courant
#+end_src

** Shared expense
Suppose I pay the full amount for an expense but part of is shared with someone
else.

#+begin_src ledger
2026-03-03 * Tap Air Portugal
    Expenses:Travel:Flights                   504.38 EUR   ; posting with full expense
    Expenses:Travel:Flights                  -252.19 EUR   ; remove from the full expense the part owed by borrower
    Liabilities:FR:Amex:Gold                 -504.38 EUR   ; posting with the source of assets and the full amount
    Assets:Receivables                        252.19 EUR   ; posting to a receivables account with the amount owed by borrower
#+end_src
...
- For receivables and payables, what matters is the counterparty, not the
  ...
  France or you repay a friend in London. -> no need for a country prefix here -
  it does not add semantic value.
* Tracking property Vincennes
** Design choices
- Use child accounts for each partner for assets, liabilities and expenses e.g.
  Liabilities:FR:CCF:Mortgage:Jean
...
- The use of child accounts allows accurate reporting of what belongs to whom in
  ...
  money and not something that is subdivided. The child division makes it easier
  to report on split of property-related expenses such as service charges,
...
Sum contributions towards liabilities.
...
** Compute monthly report for e-mail
Balance sheet:

#+begin_src sh
hledger -f huchon.journal --real close --retain --close-acct=Equity:Retained-Earnings | hledger -f huchon.journal -f - bse -I --layout=tall --real -o test.html
#+end_src

Income statement:
#+begin_src sh
hledger -f huchon.journal is --layout=tall --real -o test.html
#+end_src

* Useful commands
** Report balance sheet with earnings retained on the fly
#+begin_src sh
hledger -f all.journal close --retain --close-acct=Equity:Retained-Earnings | hledger -f all.journal -f - bse -I --layout=tall
#+end_src
* Tags
List expenses related to the value of a particular tag, for example tag =trip=
with value =lisbon-2026=
#+begin_src sh
hledger reg tag:trip=lisbon-2026
#+end_src
* General ressources
