...
Suppose we use this representation:
#+begin_src ledger
2024-09-18 * Buy LQQ
...
Indeed, with the representation below, the conversion accounts act as a "bridge"
so that the sum of postings sums to 0 independently for each commodity.  In
particular, the conversion account balance (converted to the base currency) must
be zero. This allows for strict checks.
...
- For receivables and payables, what matters is the counterparty, not the
  ...
  -> no need for a country prefix here - it does not add semantic value.
* Useful queries
